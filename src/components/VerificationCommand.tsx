...
const pendingVerifications = [
  { id: 'V-8291', user: 'NewUser#1234', timestamp: '2m ago', type: 'Math Captcha', status: 'pending' },
  { id: 'V-8292', user: 'Trader#9999', timestamp: '5m ago', type: 'Image Select', status: 'pending' },
  { id: 'V-8293', user: 'Buyer#5678', timestamp: '12m ago', type: 'Math Captcha', status: 'pending' },
];
                ...
                <div key={v.id} className="px-6 py-4 flex items-center justify-between hover:bg-white/5 transition-colors group">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-bg-tertiary border border-border overflow-hidden">
                      <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${v.user}`} alt="" referrerPolicy="no-referrer" />
                    </div>
                    <div>
                      <p className="text-sm font-bold group-hover:text-white transition-colors">{v.user}</p>
                      <div className="flex items-center gap-2 text-[10px] text-text-secondary">
                        <span className="bg-bg-tertiary px-1.5 py-0.5 rounded border border-border uppercase tracking-widest">{v.type}</span>
                        <span>•</span>
                        <span className="uppercase tracking-widest">{v.timestamp}</span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="glass p-6 rounded-2xl border border-border">
              ...
              <div className="w-full h-2 bg-bg-tertiary rounded-full overflow-hidden">
                <div className="h-full bg-white" style={{ width: '94.2%' }}></div>
              </div>
              <p className="text-[10px] text-text-secondary mt-4 uppercase tracking-widest">Based on 1,240 verification attempts in the last 24 hours.</p>
            </div>
            ...
            <div className="glass p-6 rounded-2xl border border-border">
              ...
              <p className="text-[10px] text-text-secondary mt-4 uppercase tracking-widest">Potential bot raids detected and mitigated: 12</p>
            </div>
          </div>
        </div>
